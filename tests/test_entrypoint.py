...
def test_query_command_rejects_http_registry_base_with_packet(tmp_path: Path) -> None:
    cli_main = importlib.import_module("cpm_cli.main")
    code = cli_main.main(
        [
            "query",
...
def test_query_command_rejects_http_hub_registry_endpoint(
    tmp_path: Path
) -> None:
    cli_main = importlib.import_module("cpm_cli.main")
